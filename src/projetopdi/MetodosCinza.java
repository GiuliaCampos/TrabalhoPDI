...
public class MetodosCinza {
        ...
        for (int i = 0; i < nLinhas; i++) {
            for (int j = 0; j < nColunas; j++) {
                if ((255 - matriz[i][j]) < 0) {
                    ...
                    matrizResultado[i][j] = 255 - matriz[i][j];
                }
            }
        }
    }
    ...
    public void clarearMatriz(int valor) {
        ...
        for (int i = 0; i < nLinhas; i++) {
            for (int j = 0; j < nColunas; j++) {
                if ((matriz[i][j] + valor) > 255) {
                    matrizResultado[i][j] = 255;
                } else {
                    matrizResultado[i][j] = matriz[i][j] + valor;
                }
            }
        }
    }
    ...
    //Fatiamento da Imagem - mantendo os valores de fora do intervalo
    ...
    //Fatiamento da Imagem - trocando os valores de fora do intervalo
    public void fatiamento2Imagem(int a, int b, int novaTonalidadeIntervalo, int novaTonalidadeFora){
        matrizResultado = new int[nLinhas][nColunas];
        
        for(int i = 0; i < nLinhas; i++){
            for(int j = 0; j < nColunas; j++){
                if(matriz[i][j] >= a && matriz[i][j] <= b){
                    matrizResultado[i][j] = novaTonalidadeIntervalo;
                }
                else{
                    matrizResultado[i][j] = novaTonalidadeFora;
                }
            }
        }
    }
    ...
    //Soma a imagem original com a imagem resultado
    public void somarImagens() {
        ...
        for (int i = 0; i < nLinhas; i++) {
            for (int j = 0; j < nColunas; j++) {
                ...
                if ((matriz[i][j] + matrizResultado[i][j]) >= 255) {
                    ...
                    matrizNova[i][j] = matriz[i][j] + matrizResultado[i][j];
                }
            }
        }
        ...
        for (int i = 0; i < nLinhas; i++) {
            for (int j = 0; j < nColunas; j++) {
                matrizResultado[i][j] = matrizNova[i][j];
            }
        }
    }
    
    //subtrai a imagem original da imagem resultado (original - resultado)
    public void subtrairImagens(){
        int matrizNova[][] = new int[nLinhas][nColunas];
    ...
    public void media(int filtro) {
        ...
        for (int i = 0; i < nLinhas; i++) {
            for (int j = 0; j < nColunas; j++) {
                ...
                if ((int) (soma / media) > 255) {
                    matrizResultado[i][j] = 255;
                } else {
                    matrizResultado[i][j] = (int) (soma / media);
                }
                soma = 0;
            }
        }
    ...
    public void mediana(int filtro){
        matrizResultado = new int [nLinhas][nColunas];
        int auxFiltro = filtro/2;
        int centro = 0;
        ArrayList<Integer> auxMediana = new ArrayList<>();
            ...
            for(int j = 0; j < nColunas; j++){
                for(int auxI = - auxFiltro; auxI < filtro - auxFiltro; auxI++){
                    for(int auxJ = -auxFiltro; auxJ < filtro - auxFiltro; auxJ++){
                        if((i + auxI >= 0) && (i + auxI < nLinhas)){
                            if((j + auxJ >= 0) && (j + auxJ < nColunas)){
                                auxMediana.add(matriz[i + auxI][j + auxJ]);
                            }
                        }
                    }
                }
                Collections.sort(auxMediana);
                centro = auxMediana.size() / 2;
                matrizResultado[i][j] = auxMediana.get(centro);
                auxMediana.clear();
            }
        }
    }
    ...
    public void matrizOriginalRecebeMatrizResultado() {
        int nLinhasNova, nColunasNova;
        nLinhasNova = matrizResultado.length;
        nColunasNova = matrizResultado[0].length;

        matriz = new int[nLinhasNova][nColunasNova];
    ...
    public void multiplicarValor(int valor) {
        matrizResultado = new int[nLinhas][nColunas];
        for (int i = 0; i < nLinhas; i++) {
            for (int j = 0; j < nColunas; j++) {
                if ((matriz[i][j] * valor) > 255) {
                    matrizResultado[i][j] = 255;
                } else if ((matriz[i][j] * valor) < 0) {
                    matrizResultado[i][j] = 0;
                } else {
                    matrizResultado[i][j] = matriz[i][j] * valor;
                }
